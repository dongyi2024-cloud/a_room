...
- `openspec/project.md`
目前在P1开发阶段，prd参照：docs\product/P1_PRD.md
...
## 当前视觉配色规则
当前首页与工作台视觉以黑白为主，浅紫色作为唯一强调色：
- 黑色：`#1E1D1B`
- 白色 / 半透明白：用于卡片、浮层和主要可读内容背景
- 浅紫色：`#D8C6D8`
- 深紫色：`#5E3A5F`

具体规则：
- 用户选中文字的高亮底色必须使用浅紫色，不使用黄色或暖棕色。
- 首页作者状态卡片使用黑白配色，卡片背景为约 50% 透明白底。
- 首页作者状态卡片为直角，不使用圆角。
- 首页作者状态卡片应保持紧凑上下留白，不使用大面积内边距。
- 首页右上角菜单角标使用深紫色，三道菜单杠为白色。
数据库与鉴权：
- Supabase
- Postgres
- Supabase Auth
